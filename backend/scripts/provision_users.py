...
async def provision_one(username: str, password: str, role: Role) -> None:
    settings = get_settings()
    try:
        await admin_create_user(username, password, role, settings)
        print(f"provisioned {username} ({role.value})")
...
async def main() -> None:
    ...
    if args.from_json:
        ...
        for account in accounts:
            await provision_one(
                account["username"], account["password"], Role(account["role"])
            )
    elif args.username and args.password and args.role:
        await provision_one(args.username, args.password, Role(args.role))
    else:
